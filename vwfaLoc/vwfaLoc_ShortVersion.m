function vwfaLoc_ShortVersion(SUBJ_ID,NUM,RUN,varargin)
...
DATA_DIR = [pwd '/data_ShortVersion'];
STIM_DIR = [pwd '/stimuli'];
SCREEN_NUM = max(Screen('Screens'));
FONT_SIZE = 100;
IMAGE_FORMAT = 'png';
IMAGES_PER_BLOCK = 24;
NUM_BLOCKS = 16; % If you change this you should change cb too.
if practice
    IMAGES_PER_BLOCK = 4;
...
Screen('Preference', 'SkipSyncTests', 1); %so it works
Screen('Preference', 'SuppressAllWarnings',1); %these messages are so annoying
Screen('Preference', 'Verbosity',0); %see above
Screen('Preference', 'VisualDebugLevel',0); %no scary flashing exclamation mark
